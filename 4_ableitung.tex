\chapter{Ableitung des Modells}\label{sec:ableitung}
In diesem Kapitel werden die Ergebnisse der Arbeit zusammengefasst und ein Umsetzungsleitfaden entwickelt. %TODO Falsche Trennung Umsetzungslei-tfaden !!
...
Die Fragen werden zum Schluss jedes Themenkomplexes formuliert und auf Abbildung \ref{fig:leitfaden} % DONE Ref & Label
zusammengefasst. 
...
\subsection*{Der Lean Ramp-Up Ordnungsrahmen}
 Der Lean Ramp-up Ordnungsrahmen wie auf Abbildung \ref{fig:dom17a:ordnungsrahmen} zu sehen, hat die Aufgabe, die Gestaltung der Zielerreichung des Serienanlaufs auf verschiedenen Abstraktionsebenen darzustellen und zu gestalten. 
...
\begin{itemize}
  \item Wie kann der Serienanlauf inhaltlich strukturiert werden? 
  \item Was sind die kritischen Themenstellungen im Serienanlauf?
  \item Welche (Teil-)Ziele verfolgt der Serienanlauf? 
    \begin{itemize}
    \item In welcher Beziehung stehen sie zueinander? 
    \end{itemize}
  \item Welche Aufgaben unterstützen die Zielerreichung? 
    \begin{itemize}
    \item Welchen Methoden, Werkzeugen und Mitarbeitern (Rollen) können sie zugeordnet werden?
    \end{itemize}
...
\begin{itemize}
  \item Anhand welcher Kriterien / Kennzahlen lässt sich der Reifegrad des Produkts / der Prozesse quantifizieren? 
  \item Zu welchem Zeitpunkt müssen Produktdaten digital vorliegen? 
    \begin{itemize}
    \item Welche Detailtreue und -qualität ist wann zu wählen?
    \end{itemize}
  \item An welchen Stellen ist Standardisierung sinnvoll? 
\end{itemize}
...
\begin{itemize}
\item Mit welcher Strategie soll auf Störungen im Anlauf reagiert werden? 
  \begin{itemize}
  \item Welche Methoden, Werkzeuge, Standards können hierfür definiert werden? 
  \end{itemize}
\item Welche Anlaufindikatoren können zur Überwachung definiert werden? 
  \begin{itemize}
  \item Wie könnte eine Scorecard zur Überwachung aussehen? 
  \end{itemize}
\item Mit welchen Modellen können komplexe Systeme strukturiert und
abstrahiert werden? 
\end{itemize}
...
\begin{itemize}
\item Wie können Produkt- und Prozessdaten verknüpft werden? 
  \begin{itemize}
  \item Welche Software eignet sich dafür?
  \end{itemize}
\item Wie können Folgen von Änderungen simuliert und bewertet werden?
\end{itemize}
...
\begin{itemize}
\item Wie können die Kundenwünsche erfasst werden? 
  \begin{itemize}
  \item Wie können mangelhafte Produkteigenschaften identifiziert werden? 
  \item Welche Erhebungsinstrumente eigenen sich dafür?
  \end{itemize}
\item Wie können Lernprozesse zur Reduzierung der Time-to-market eingesetzt werden? 
\end{itemize}
...
\begin{itemize}
\item Wie können komplexe Ursache-Wirkungs-Zusammenhänge analysiert werden? 
\item Welche Dimensionen eignen sich für die Ishikawa-Analyse? 
\item Mit welchen Phasen soll das Ishikawa-Diagramm erweitert werden?
\end{itemize}
...
Während die Kostenrisiken direkt quantifiziert werden können, müssen Leistungsrisiken im Rahmen der Szenarioberechnung bewertet werden (s. Abschnitt \ref{sec:wildemannszenarien}).
...
\begin{itemize}
\item Anhand welcher Kriterien sollen potentielle Risiken gruppiert werden?
  \begin{itemize}
  \item Welchen Produktionssystem-Elementen lassen sie sich zuordnen? 
  \end{itemize}
\item Welche Handlungsstrategien können für Risikogruppen entwickelt werden?
\item Anhand welcher Merkmale sollen Risiken bewertet werden?
\item Welche Risiken sollen laufend überwacht werden?
\end{itemize}
...
\begin{itemize}
\item In welche Phasen können Änderungsstrategien untergliedert werden?
  \begin{itemize}
  \item Wie sollen jeweils Änderungsprozesse gestaltet werden?
  \end{itemize}
\item Durch welche Maßnahmen können Änderungsprozesse beschleunigt werden?
\item Wie können änderungsbedingte Arbeitsabläufe strukturiert werden? 
\end{itemize}
...
Um schnell auf Änderungen reagieren zu können, muss der Planungsaufwand reduziert werden. Zentrale Aspekte bilden hier die Einführung und Nutzung von Standards, welche die Komplexität (Auswahl) verringern. Für Maschinen und Anlagen könnte \gls{bspw} eine Standardisierung mit Hilfe von Bibliotheken erfolgen. Routinetätigkeiten wie z.B. Auswertungen sollten automatisiert erfolgen. 
  ...
  \begin{itemize}
  \item Welche Elemente können standardisiert oder modularisiert werden?
  \item Wie können Fertigungssysteme transparent und flexibel gestaltet werden? 
  \item Wie können Fertigungssysteme effizient an ständig wechselnde Anforderungen angepasst werden? 
  \end{itemize}

\begin{figure}[ht]
 \centering
 \includegraphics[angle=90,scale=.8, page=3, trim=20 20 20 20]{./img/Konzept.pdf}
